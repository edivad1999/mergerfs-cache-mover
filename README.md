...
- `STATUS_PATH`: The path for the structured status file consumed by the WebUI.
...
- `WEB_UI_HOST`: WebUI bind address (default `0.0.0.0` in Docker)
- `WEB_UI_PORT`: WebUI port (default `9090`)
...
- `STATUS_PATH`: Path for the WebUI status JSON file (default: `/var/log/cache-mover-status.json`)
- `SCHEDULE`: Cron expression for scheduling (default: "0 3 * * *")
- `WEB_UI_HOST`: WebUI bind address (default: `0.0.0.0`)
- `WEB_UI_PORT`: WebUI port (default: 9090)
...
### WebUI in Docker
The Docker container starts a monitor-only WebUI automatically on port `9090`. It shows service health, cache and backing usage, current settings, last run status, recent logs, and a manual deep scan of movable files.
...
Open `http://localhost:9090` after publishing the port. The WebUI has no authentication by design, so do not expose this port to the public internet.

The WebUI reads recent logs from `LOG_PATH` and structured run state from `STATUS_PATH`. By default, both live under `/var/log`, so mounting `/var/log` persists dashboard history across container restarts.
